...
RapidlyProcessorHandle g_processorHandle = nullptr;
...
void handleError (string errorMessage)
{
    ...
    if (g_processorHandle != nullptr)
        rapidlyDeleteProcessor (g_processorHandle);
...
int main (int argc, char* argv[])
{
    ...
    g_processorHandle = rapidlyCreateProcessor (modelFilePath, numOfChannels, sampleRate);
    if (g_processorHandle == nullptr)
        handleError ("Unable to create the Rapidly audio processor.");

    // Print model information
    RapidlyProcessorInfo processorInfo;
    rapidlyGetProcessorInfo (g_processorHandle, &processorInfo);
    ...
    rapidlyGetVectorArithmeticWrapperName ((char*) wrapperNameBuffer.data(), wrapperNameBuffer.size());
    ...
    for (int busIndex = 0; busIndex < numOfOutputBusses; busIndex++) {
        ...
        rapidlySetParameterValue (g_processorHandle, RAPIDLY_PARAM_BUS_SENSITIVITIES + busIndex, 0.f);

        // Read back the parameter values for demonstration
        float gain          = rapidlyGetParameterValue (g_processorHandle, RAPIDLY_PARAM_BUS_GAINS + busIndex);
        float sensitivity   = rapidlyGetParameterValue (g_processorHandle, RAPIDLY_PARAM_BUS_SENSITIVITIES + busIndex);
    ...
    while (numOfSamplesWritten < numOfSamplesInSource) {
        ...
        rapidlyAddAudioInterleaved (g_processorHandle, audioBuffer.data(), numOfSamplesInBuffer);

        // Now, we check how many samples that are ready for pick-up
        int numOfSamplesToWrite = min (rapidlyGetNumOfPendingSamples (g_processorHandle), numOfSamplesInSource - numOfSamplesWritten);
        ...
        if (numOfSamplesToWrite > 0) {
            ...
            if (!rapidlyGetAudioInterleaved (g_processorHandle, processedBuffer.data(), numOfSamplesToWrite))
    ...
    // Close the file handles and delete the Rapidly processor
    fclose (g_inputFileHandle);
    fclose (g_outputFileHandle);
    rapidlyDeleteProcessor (g_processorHandle);
