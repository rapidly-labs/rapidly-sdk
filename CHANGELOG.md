...
- Kotlin / Android binding `rapidly-engine-1.0.aar`, available as `implementation("io.rapidly:engine:1.0")` from Maven Central or as a downloadable artefact in the GitHub release. Kotlin wrapper source mirrored at `bindings/kotlin/`. Bundles `arm64-v8a`. `minSdk` 26.
