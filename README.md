...
| Folder | Contents |
|---|---|
| `bin/` | Native binaries, one folder per platform |
...
The SDK enforces licence entitlements locally; no network access is required. A Rapidly licence (`lk_...`) is issued by Rapidly Labs and encodes which models and latency variants your application is permitted to load. Without a valid licence, the SDK operates in a watermarked demo mode that audibly degrades its output.
...
Linux x64, Linux arm64, Windows x64, Windows x86.

macOS, iOS, Android arm64, and WebAssembly are coming in a future release. See `CHANGELOG.md` for the full v1.0 scope.
