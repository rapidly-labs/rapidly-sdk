...
class RapidlyEngine:
    """
    The RapidlyEngine is a machine learning audio separation module
    ...
    def find_binary(self) -> str:
        """
        Returns the path to the engine binary in the bin folder. The bin
        folder layout matches the public SDK Distribution (lowercase
        platform subdirs: windows-x64, windows-x86, macos, linux-x64,
        linux-arm64). macOS ships as a universal dylib covering both
        ...
        if system == "Windows":
            if platform.architecture()[0] == "32bit":
                path_to_binary = os.path.join(relative_path, "windows-x86", "RapidlyEngine.dll")
            else:
                path_to_binary = os.path.join(relative_path, "windows-x64", "RapidlyEngine.dll")
        elif system == "Darwin":
            macos_dir = os.path.join(relative_path, "macos")
            fn = self.find_preferred_dylib(macos_dir, "libRapidlyEngine")
            if fn:
                path_to_binary = os.path.join(macos_dir, fn)
        elif system == "Linux":
            if machine in ("aarch64", "arm64"):
                path_to_binary = os.path.join(relative_path, "linux-arm64", "libRapidlyEngine.so")
            else:
                path_to_binary = os.path.join(relative_path, "linux-x64", "libRapidlyEngine.so")
